...
##### tblock ###### 
    ...
    Primitive('moveOntoTemplateBlock', arrow(tblock, tcolor, tcolor), _, 97a05b5b) # tcolor to tcolor bijection
    Primitive('applyMask', arrow(tblock, tmask, tblock), _, 80af3007) # tblock and tmask must be the exact same size
    Primitive('colorByClosestTile', arrow(tblock, tblocks, tblock), _, _)
    Primitive('extendColorUntilEdge', arrow(tblock, tcolor, tdirection, tblock), _, b8cdaf2b) # extend in tdirection if tile not part of block
    ...
    Primitive('hasColorNeighbor', arrow(tblock, tcolor, tbool, tbool), _, 9edfc990) # if tbool shared corner suffices for neighbor, otherwise shared edge
...
##### ttile #####
    ...
    Primitive('extendUntilSingleDirection', arrow(ttile, tdirection, arrow(tblock, tbool), tblock), _, 2c608aff), # condition given by function
    Primitive('extendTowardsTile', arrow(ttile, ttile, tblock), _, _),
    ...
    # arrow(ttile, tbool)
    Primitive('HasSharedBlockEdges', arrow(ttile, tint, tblock, tbool), _, 7e0986d6), #tint: minimum number of shared edges

##### ttiles #####

    Primitive('createGrid', arrow(ttiles, tint, tbool, tgrid), _, cdecee7f), # tbool: whether to build snakewise, tint: numRows

##### tsquare #####

    # arrow(tsquare, tsquare)
    Primitive('fillAlternatingPattern', arrow(tsquare, tcolor, tcolor, tbool, tsquare), _, b60334d2), # if tbool start with first tcolor
...
##### tgrid #####
    ...
    Primitive('findMinWrappingRectanglesBlackB', arrow(tgrid, tblocks), _, 80af3007), # includes all tiles in the rectangle
    ...
    Primitive('findLooseBlocks', arrow(tgrid, tint, arrow(tgrid, tblocks)), tblocks, _, 0b148d64-a64e4611), # adjacent tiles are at most tint away. background
